...
from UI.DisplayBoard import AnalyizeDataUI
...
class ImageDetectionUI(QMainWindow):
    ...
    model_paths = [
        ...
        r"retrain_runs\read_lean table_stand\read_lean table_stand\weights\best.pt"
    ]
    def __init__(self):
        super().__init__()
        self.load_models()
    ...
    def initOptionUI(self):
        ...
        """
        optionLayout = self.findChild(QVBoxLayout, "option_layout")
        self.ana_button = CyberButton("分析结果", self.option, icon_path="assets/moniter_detect.png")
        optionLayout.insertWidget(12, self.ana_button, alignment=Qt.AlignHCenter)
        self.ana_button.setFixedSize(self.parentWidth, self.parentHeight)
    ...
    def initConnection(self):
        ...
        绑定分析结果按钮
        """
        self.ana_button.clicked.connect(self.analyize_json) # 显示分析结果界面

    def analyize_json(self):
        """
        分析json文件
        """
        # get the json file
        json_file_name, _ = QFileDialog.getOpenFileName(self, "选择json文件", "", "Json Files (*.json)")
        logger.info(f"选择的json文件为：{json_file_name}")
        if not json_file_name:
            logger.error("没有选择json文件")
            return
        self.board = AnalyizeDataUI(json_file_name)
        self.board.show()
